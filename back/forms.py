...
CLASS_FILE = 'ml-5 bg-white py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500'
...
class UserForm(ModelForm):
    class Meta:
        model = User
        fields = ['username', 'first_name', 'last_name', 'email', 'picture_profile', 'is_active', 'is_staff', 'is_superuser']
        widgets = {
            'username': forms.TextInput(attrs={'class': CLASS_INPUT}),
            'first_name': forms.TextInput(attrs={'class': CLASS_INPUT}),
            'last_name': forms.TextInput(attrs={'class': CLASS_INPUT}),
